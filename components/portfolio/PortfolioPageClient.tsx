...
export default function PortfolioPageClient({ content, lang }: PortfolioPageClientProps) {
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="flex flex-wrap justify-center gap-3 mb-12">
              {content.categories.map((category) => (
                <button
                  key={category}
                  className="px-6 py-2 rounded-full border-2 border-blue-600 text-blue-600 hover:bg-blue-600 hover:text-white transition-all duration-300 font-medium dark:border-blue-400 dark:text-blue-400 dark:hover:bg-blue-400 dark:hover:text-white"
                >
                  {category}
                </button>
              ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto">
              {content.items.map((item) => (
                <div
                  key={item.id}
                  onClick={() => handleProjectClick(item as Project)}
                  className="bg-white dark:bg-gray-800 rounded-lg overflow-hidden shadow-md hover:shadow-xl transition-all duration-300 group cursor-pointer"
                >
                  {/* Project Image */}
                  <div className="relative aspect-[4/3] bg-gradient-to-br from-blue-100 to-blue-200 dark:from-blue-900 dark:to-blue-800 overflow-hidden">
                    {item.thumbnail ? (
                      <img
                        src={item.thumbnail}
                        alt={item.title}
